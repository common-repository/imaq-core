jQuery(document).ready(function ($) {
    var $form = $('#IMAQ_url_settings_form');
    $form.on('submit', function (e) {
        e.preventDefault();
        var data = $(this).serialize();
        $.post(ajaxurl, data)
            .then(function (response) {
                $('#IMAQ_url_settings_form').before('<div class="updated notice is-dismissible"><p>' +response.data.message+ '</p>' +
            ...
            .fail(function (error) {
                $('#IMAQ_url_settings_form').before('<div class="error notice is-dismissible"><p>' +error.responseJSON.data.message+ '</p>' +
                    '<button id="dismiss-admin-message" class="notice-dismiss" type="button">' +
                    '<span class="screen-reader-text">Dismiss this notice.</span></button></div>');
                $("#dismiss-admin-message").click(function(e) {
                    e.preventDefault();
